class Api::EverydayBehaviorsController < ApplicationController
  ...
  def create
    @everyday_behavior = current_user.everyday_behaviors.build(everyday_behaviors_params)
  ...
  def update
    if @everyday_behavior.update(everyday_behaviors_params)
      render json: @everyday_behavior
    else
      render json: @everyday_behavior.errors, status: :bad_request
    end
